    ...
    >
      <Card.Body>
        <Card.Title>
          ...
          <Row>
            <Col>
              <Badge style={{ float: "left" }} bg={stateColours[status]}>
                {showStatus
                  ? status
                  : status === ABANDONED
                  ? game.reason
            ...
            <Col>
              <Badge style={{ float: "right" }} bg={stateColours[status]}>
                {status === WISH_LIST
        ...
        <Card.Text>
          <Row>
            {(game.progress || game.completed) && (
              <ProgressBar
                className="mb-3"
                now={game.completed ? 100 : game.progress}
                label={`${game.completed ? "100" : game.progress}%`}
              />
            )}
          </Row>
          <Row>
            <Col className="card-icon">
              {game.isCoop && (
                <div
                  style={{ paddingLeft: "20%", width: "50%", float: "left" }}
                >
                  <Joystick size={20} />
                </div>
              )}
              <div
                style={{
                  paddingRight: game.isCoop ? "20%" : "0px",
                  width: game.isCoop ? "50%" : "100%",
                  float: "left",
                }}
              >
                <Joystick size={20} />
              </div>
              <span>{getPlatform(game.platform)}</span>
            </Col>
            <Col
              className="card-icon"
              style={{
            ...
            <Col className="card-icon">
              <Collection size={20} />
              <span>{restrictGameProperty((game.genres || [])[0])}</span>
            </Col>
          </Row>
        </Card.Text>
      </Card.Body>
