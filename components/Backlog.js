...
const sortOptions = [
  ...
  {
    label: "Score",
    labelTransform: (x) => convertBacklogScoreToString(x),
...
export default function Backlog() {
  ...
  const { selectedState, setSelectedState, showAddGame, showSearch } =
    useRouter();
  ...
  const platforms = [
    ...new Set(
      backlog.games
        .filter((x) => isInState(x, selectedState))
        .map((x) => x.platform)
    ),
  ];
  ...
  const sort =
    selectedSort &&
    (!selectedSort.status || selectedSort.status === selectedState)
      ? selectedSort
      : sortOptions.find(
          (x) => x.property === defaultSortByState[selectedState]
        ) || sortOptions[0];
  ...
  return (
    <div>
      ...
      <Container className="mainContainer">
        ...
        >
          {sortOptions
            .filter((x) => !x.status || x.status === selectedState)
            .map(({ label, property, direction, grouped }, index) => (
              <Dropdown.Item
                key={index}
                style={{ zIndex: 9999 }}
                onClick={() => {
                  setSort({
                    ...
                    direction:
                      sort.property === property
                        ? getOppositeSortDirection(sort.direction)
                        : direction,
                    grouped,
                  });
                }}
              >
                {getSortIcon(
                  sort.property === property && sort.direction == direction
                    ? getOppositeSortDirection(direction)
                    : direction
                )}{" "}
                {label}
              </Dropdown.Item>
        ...
        >
          <Dropdown.Item
            key="any"
            onClick={() => setSelectedOwnedAs(null)}
            style={{ zIndex: 9999 }}
          >
            Any
          </Dropdown.Item>
          {ownershipTypes.map((type, index) => (
            <Dropdown.Item
              key={index}
              onClick={() => setSelectedOwnedAs(type)}
              style={{ zIndex: 9999 }}
            >
              {type}
            </Dropdown.Item>
          ...
          >
            {selectedBoolFilters.map((filter, index) => (
              <Dropdown.Item
                key={index}
                onClick={() =>
                  setSelectedBoolFilters(
                    selectedBoolFilters.map((x) =>
                      x.label === filter.label ? { ...x, on: !x.on } : x
                    )
                  )
                }
                style={{ zIndex: 9999 }}
        ...
        <Carousel
          ...
          onSelect={(index) => setSelectedState(states[index])}
        >
          {states.map((state, index) => {
            const previousState = index > 0 ? states[index - 1] : null;
            const nextState =
              index < states.length - 1 ? states[index + 1] : null;

            let games = backlog.games
              .filter((x) => isInState(x, state))
              .filter(
                (x) => !selectedPlatform || x.platform === selectedPlatform
              )
            ...
            if (selectedState === PLAYED) {
              ...
              games = games.filter((x) =>
                activeFilters.some((filter) => filter.filter(x))
              );
            }
            ...
            if (!sort.grouped) {
              ...
              games.forEach((x) => {
                ...
                const g = gamesByGroup.find((x) => group === x.group) || {
                  group,
                  games: [],
                };
              ...
              <Carousel.Item>
                ...
                >
                  <Stack direction="horizontal" gap={1}>
                    {previousState && (
                      <Button
                        variant={"outline-" + stateColours[previousState]}
                        size="sm"
                        onClick={() => setSelectedState(previousState)}
                      >
                        <ArrowLeft /> {previousState}
                      </Button>
                    )}
                    <Button
                      variant={"outline-" + stateColours[state]}
                      size="sm"
                    >
                      {state}{" "}
                      <Badge bg={stateColours[state]}>{games.length}</Badge>
                    </Button>
                    {nextState && (
                      <Button
                        variant={"outline-" + stateColours[nextState]}
                        size="sm"
                        onClick={() => setSelectedState(nextState)}
                      >
                ...
                {!sort.grouped && (
                  <div>
                    <h6>
                      Ordered by {sort.label}{" "}
                      {sort.direction == "asc" ? <ArrowDown /> : <ArrowUp />}
                    </h6>
                    {games.map((game) => (
                ...
                {sort.grouped &&
                  gamesByGroup.map(({ group, games: groupGames }) => {
                    return (
                      <div>
                        <h6>
                          {sort.labelTransform
                            ? sort.labelTransform(group)
                            : group}{" "}
                          <Badge bg={stateColours[state]}>
                            {groupGames.length}
                          </Badge>
                        </h6>
                        {groupGames.map((game) => (
