...
import { convertBacklogScoreToInt } from "./utils.js";

const handler = async (req, res) => {
  const state = req.body;
  const { results: existingGames } = await datagraphs.concepts.search({
  ...
  const toDelete = existingGames.filter(
    (x) => !games.some((y) => y.id === x.id)
  );
