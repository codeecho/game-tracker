...
export default function BacklogProvider({ children }) {
    ...
    const add = (game, status) => updateState({ ...state, games: state.games.concat({ ...game, status }) });

    const update = (game) => updateState({ ...state, games: state.games.map(x => {
    ...
    return (
        <backlogContext.Provider value={{ backlog: state, add, update, remove, reset }}>
            {children}
